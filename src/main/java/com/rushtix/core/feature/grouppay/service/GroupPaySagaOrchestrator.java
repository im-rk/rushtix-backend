...
public class GroupPaySagaOrchestrator {
    ...
    public void commitSaga(GroupBooking saga) {
        log.info("Saga Condition MET. Committing group booking tickets: {}", saga.getId());
    ...
    public void rollbackSaga(UUID groupBookingId) {
        ...
        // 1. Release database and Redis holds safely
        saga.getPaymentItems().forEach(item -> {
            Seat seat = seatRepository.findById(item.getAssignedSeatId()).orElseThrow();
        ...
        saga.getPaymentItems().stream()
                ...
                .forEach(item -> {
                    String intentId = item.getStripePaymentIntentId();
                    UUID itemId = item.getId();

                    item.setStatus(SplitStatus.REFUNDED); // Update status inside the transaction boundary

                    CompletableFuture.runAsync(() -> {
                        try {
                            paymentGatewayService.refundCheckoutPayment(intentId);
                            log.info("Compensating partial refund executed successfully on Stripe for entry item: {}", itemId);
                        } catch (Exception e) {
                            log.error("CRITICAL ERROR: Automated compensation breakdown on Stripe intent {}", intentId, e);
                        }
                    });
                });
